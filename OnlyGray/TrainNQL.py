...
Transition = namedtuple('Transition',
                        ('sgray','action','next_sgray','reward'))
...
class TrainNQL:
	...
	def get_data(self,episode,tsteps):	
		images = []
		dirname_rgb='dataset/Depth/ep'+str(episode)
	...
	def load_data(self):
		...
		for i in best_scores:
			print('Ep: ',i+1)
			dirname_gray='dataset/Depth/ep'+str(i+1)
			files = []
			...
			for step  in range(k-1):
				...
				self.memory.push(image,action,next_image,reward)
	...
	def train(self):
		...
		for i in range(0,self.bufferSize,self.minibatch_size):
			...
			for t in transitions:
				...
				count = 0
				for sgray,next_sgray in zip(t.sgray,t.next_sgray):
				...
				one_transition = Transition(proc_sgray,t.action,proc_next_sgray,t.reward)
