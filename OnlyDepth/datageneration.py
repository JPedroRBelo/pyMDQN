...
def generate_data(episode,agent,env):
	...
	terminal = 0
	screen = None
	screen reward, terminal = env.perform_action('-',init_step+1)
	...
	while step <=t_steps+1:
		...
		action_index = agent.perceive(screen, terminal, False, numSteps,step,testing)		
		step=step+1		
		if action_index == None:
				action_index=1
		if not terminal: 
			screen,reward,terminal=env.perform_action(aset[action_index],step)
		else:
			screen, reward, terminal = env.perform_action('-',step)
