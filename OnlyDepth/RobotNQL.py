...
class RobotNQL:
	...
	def perceive(self, state, terminal, testing, numSteps, steps, testing_ep):
		curState = state.to(self.device)
		actionIndex = 0
		if not terminal:
			actionIndex = self.eGreedy(curState, numSteps, steps, testing_ep)
	...
	def eGreedy(self,state, numSteps , steps, testing_ep):
